...
def _strip_html_for_matching(value: str | None) -> str:
    text = _decode_display_value(value) or ""
    if not text:
        return ""

    text = html_unescape(text)

    # Bevar linjeskift omkring blokke, så "Diktatord:" bedre kan matches som egen linje.
    text = re.sub(r"(?i)<\s*br\s*/?\s*>", "\n", text)
    text = re.sub(r"(?i)</\s*(div|p|li|h[1-6])\s*>", "\n", text)
    text = re.sub(r"(?i)<\s*li\b[^>]*>", "- ", text)

    # Fjern resten af HTML-tags.
    text = re.sub(r"<[^>]+>", "", text)

    # Normaliser whitespace uden at fjerne linjestruktur helt.
...
def _extract_practice_text_from_general_content(
    ...
    text = _strip_html_for_matching(content_text)
    if not text:
        return None

    lowered = text.lower()

    # Find kun linjer/afsnit hvor keywordet efterfølges af kolon,
    # så vi undgår at matche fritekst som "vi kører en omgang diktatord".
    for keyword in keywords:
        ...
        prefix_clean = re.sub(r"\s+", " ", (match.group(1) or "").strip())
        suffix_clean = re.sub(r"\s+", " ", (match.group(2) or "").strip(" :-\n\t"))
        if suffix_clean:
            return (_derive_homework_title_from_prefix(prefix_clean), suffix_clean)

    # Fallback: kig linje for linje efter "keyword:" hvis teksten er mere simpel.
